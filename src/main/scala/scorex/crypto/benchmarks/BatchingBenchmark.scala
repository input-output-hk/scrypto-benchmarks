...
/**
  * The benchmarks contains comparison of proof size and performance for batching and non-batching(legacy) modes
  */
object BatchingBenchmark extends App with TwoPartyTests {
  ...
  /**
    * Prints out sizes and generation/verification times for batching and legacy provers
    * Tree is initially populated.
    */
  def bench2(): Unit = {
    ...
    steps foreach { st =>
      ...
      oneStep(InitialMods, st, st, 1024 * 32, oldProver, newProver, mods)
    }
  }
      ...
      for (i <- 0 until n) {
        if (!halfInserts || i % 2 == 0) {
          if (useFreshLookups) {
            ...
            // successful lookup with a change
